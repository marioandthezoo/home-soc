...
#: goes, which blocklists apply, where alerts go, who is scanned, and the dashboard's own bind
#: and credential. The startup tamper warning (cli) and the "overriding config.toml" warning
#: (web) both use this one list.
TAMPER_SIGNAL_KEYS: tuple[str, ...] = (
    "web.", "notify.", "dns.upstreams", "dns.doh_upstream", "dns.lists", "dns.listen", "dns.enabled",
    "network.exclude",
)

#: Settings-table row (not a config key) recording the web.host / web.token overrides the owner
#: is known to have made: generated by the bind policy, set with the CLI, or saved on the
#: Settings page while signed in with a password. An override that does not match it was
#: written some other way (for example by another program while the dashboard had no password,
#: before that was refused) and is reported at startup.
CONFIRMED_WEB_SETTING = "bindpolicy.confirmed"
_CONFIRMED_WEB_KEYS = ("web.host", "web.token")
...
def _confirmed_record(conn: sqlite3.Connection) -> dict[str, str]:
    try:
        record = json.loads(str(db.get_setting(conn, CONFIRMED_WEB_SETTING, "") or "{}"))
    except ValueError:
        return {}
    return {str(k): str(v) for k, v in record.items()} if isinstance(record, dict) else {}


def confirm_web_overrides(conn: sqlite3.Connection, keys: Any = None) -> None:
    """Record the current web.host / web.token overrides (all, or just ``keys``) as the owner's
    own. Only fingerprints are stored, never the token."""
    stored = overrides(conn)
    record = _confirmed_record(conn)
    for key in (_CONFIRMED_WEB_KEYS if keys is None else [k for k in keys if k in _CONFIRMED_WEB_KEYS]):
        if key in stored:
            record[key] = _fingerprint(stored[key])
        else:
            record.pop(key, None)
    db.set_setting(conn, CONFIRMED_WEB_SETTING, json.dumps(record, sort_keys=True))


def unconfirmed_web_overrides(conn: sqlite3.Connection) -> list[str]:
    """web.host / web.token overrides that no trusted path is known to have written."""
    stored = overrides(conn)
    record = _confirmed_record(conn)
    return [key for key in _CONFIRMED_WEB_KEYS
            if key in stored and record.get(key) != _fingerprint(stored[key])]
...
__all__ = [
    ...
    "MIN_TOKEN_LENGTH",
    "TAMPER_SIGNAL_KEYS",
    "CONFIRMED_WEB_SETTING",
    "confirm_web_overrides",
    "unconfirmed_web_overrides",
    "is_loopback_host",
    "lan_bind_problem",
    "load",
    "build",
