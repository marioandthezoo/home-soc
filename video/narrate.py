...
def cues_for_scene(
    ...
    # boundaries[i] is where sentence i ends, relative to the first spoken sample
    boundaries: list[float] = []
    running = 0.0
    for weight in weights[:-1]:
        running += seconds * (weight / total)
        boundaries.append(running)
    boundaries = snap_to_pauses(boundaries, pauses, seconds)

    edges = [0.0, *boundaries, seconds]
    return [
        Cue(
            text=sentence,
            start=start + edges[i],
...
def narrate(
    *,
    ...
    if not 6.5 * 60 <= speech <= 8.0 * 60:
        print(
            f"  NOTE: CONTRACT §2 asks for 6.5-8 minutes of narration; this script is "
            f"{speech / 60:.2f} min.",
            file=sys.stderr,
        )
    return results
